...
void clientSide2(int port,string outputFile)throw (const char*){
	...
	int serverFD = initClient(port);
	ofstream out(outputFile);
	ifstream in("/home/maorsdf/mileStone3/input.txt");
	string input="";
	while(input!="6"){
		readMenue(out,serverFD);
		getline(in,input);			
...
int main(){
	...
	size_t mistakes = check(outputFile1,"/home/maorsdf/mileStone3/expected_output_menu.txt");
	mistakes += check(outputFile2,"/home/maorsdf/mileStone3/expected_output.txt");
